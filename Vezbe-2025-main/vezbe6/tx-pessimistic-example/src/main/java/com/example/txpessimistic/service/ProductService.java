package com.example.txpessimistic.service;

import com.example.txpessimistic.model.Product;
import com.example.txpessimistic.repository.ProductRepository;
...
public class ProductService {
    ...
    @Transactional(readOnly = false, propagation = Propagation.REQUIRES_NEW)
    public Product update(Product product) throws PessimisticLockingFailureException {

        logger.info("> update id: {}", product.getId());
        Product productToUpdate = productRepository.findOneById(product.getId());
        logger.info("Product from DB before update {}", productToUpdate.toString());
